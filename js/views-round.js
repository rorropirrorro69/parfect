...
function approachView(h, chole, G, pf, px) {
  ...
  const field = `<rect width="${W}" height="${H}" fill="url(#g3dGrass)"/>
    ...
    <line x1="${pin.x.toFixed(0)}" y1="${pin.y.toFixed(0)}" x2="${pin.x.toFixed(0)}" y2="${(pin.y - flagH).toFixed(0)}" stroke="#ffffff" stroke-width="2.2"/><path d="M${pin.x.toFixed(0)},${(pin.y - flagH).toFixed(0)} l14,4 -14,4z" fill="#4fc07a"/>
    <ellipse cx="150" cy="262" rx="92" ry="26" fill="${lieCol}"/>
    <ellipse cx="150" cy="256" rx="56" ry="13" fill="#ffffff" opacity="0.08"/>`;
  const landDot = `<ellipse cx="${land.x.toFixed(0)}" cy="${(land.y + 2).toFixed(0)}" rx="4.4" ry="1.7" fill="#000" opacity="0.2"/><circle cx="${land.x.toFixed(0)}" cy="${land.y.toFixed(0)}" r="4" fill="${ok ? '#4fc07a' : '#e3c887'}" stroke="#16301a" stroke-width="0.8"/>`;
...
function greenCloseup(h, G, pf, px) {
  ...
  const field = `<rect width="${W}" height="${H}" fill="url(#g3dGrass)"/>
    ...
    <line x1="${pin.x.toFixed(0)}" y1="${pin.y.toFixed(0)}" x2="${pin.x.toFixed(0)}" y2="${(pin.y - 42).toFixed(0)}" stroke="#ffffff" stroke-width="2.5"/><path d="M${pin.x.toFixed(0)},${(pin.y - 42).toFixed(0)} l16,5 -16,5z" fill="#4fc07a"/>`;
...
function captureSchematic(h, chole, noZoom, clean) {
  ...
  const greenG = `${haz}${bunkers}
    ...
    <line x1="${pin.x.toFixed(0)}" y1="${pin.y.toFixed(0)}" x2="${pin.x.toFixed(0)}" y2="${(pin.y - flagH).toFixed(0)}" stroke="#ffffff" stroke-width="1.8"/><path d="M${pin.x.toFixed(0)},${(pin.y - flagH).toFixed(0)} l11,3.2 -11,3.2z" fill="#4fc07a"/>`;
