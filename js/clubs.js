...
const Clubs = (() => {
  ...
  async function pushClub(c) {
    ...
    try {
      ...
      for (const t of (c.tournaments || [])) {
        r = await db.from('tournaments').upsert({ id: t.id, club_id: c.id, name: t.name, date: t.date || null, holes: t.holes, par: t.par, format: t.format || 'stroke', status: t.status || 'live', sponsors: t.sponsors || [] });
        if (r.error) throw r.error;
  ...
  async function pull(force) {
    ...
    try {
      ...
      const tById = {};
      (tn.data || []).forEach(t => { const c = byClub[t.club_id]; if (c) { const o = { id: t.id, name: t.name, date: t.date, holes: t.holes, par: t.par, format: t.format, status: t.status, sponsors: t.sponsors || [], players: [] }; c.tournaments.push(o); tById[t.id] = o; } });
