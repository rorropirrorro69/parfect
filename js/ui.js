...
const GOLF_CAP = ['none', '#4fc07a', '#e8483a', '#2a3550', '#3a8fe0', '#f2c33a', '#ffffff', '#16241A'];
const GOLF_SHIRT = ['#4fc07a', '#e8483a', '#3a8fe0', '#ff8a3d', '#9a5cd0', '#2fa36b', '#16241A', '#ffffff'];
...
const GOLF_DEFAULT = { type: 'hombre', skin: '#f6d0aa', hair: '#2a1c12', cap: 'none', shirt: '#4fc07a', pants: '#2a3550', face: 'normal' };
...
function senseiBird(cls) {
  return `<svg class="sbird ${cls || ''}" viewBox="0 0 56 56" aria-hidden="true">
    <defs>
      <radialGradient id="sbGlow" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="#4fc07a" stop-opacity=".55"/><stop offset="55%" stop-color="#8a5cd0" stop-opacity=".24"/><stop offset="100%" stop-color="#8a5cd0" stop-opacity="0"/></radialGradient>
    ...
    <path d="M37,15 Q39,7 45,5 Q42,11 43,16 Z" fill="#4fc07a"/>
...
const OUTFITS = [
  ...
  { k: 'lime', n: 'Lima', sw: '#4fc07a', f: 'hue-rotate(72deg) saturate(1.4) brightness(1.05)' },
...
const GOLF_OUTFITS = [
  ...
  { k: 'lima', n: 'Lima', cfg: { type: 'hombre', skin: '#f6d0aa', hair: '#2a1c12', cap: '#16241A', shirt: '#4fc07a', pants: '#16241A', face: 'shades' } },
...
const GOLF_ICONS = {
  ...
  flag: `<ellipse cx="16" cy="28" rx="11" ry="2.6" fill="#2f6b39"/><ellipse cx="16" cy="27.4" rx="7" ry="1.6" fill="#57b15c"/><rect x="14.7" y="5" width="1.8" height="22" rx=".9" fill="#cdd6c2"/><g class="gi-wave" style="transform-origin:15.6px 6px"><path d="M16.2 5 L27 8.6 L16.2 12.2 Z" fill="#4fc07a"/></g><circle cx="15.6" cy="5" r="1.8" fill="#eef3e6"/>`,
  tee: `<ellipse cx="16" cy="29" rx="8" ry="2" fill="#000" opacity=".25"/><circle cx="16" cy="9" r="6.5" fill="#cdd5d7"/><circle cx="13.5" cy="6.5" r="2.2" fill="#fff" opacity=".85"/><path d="M11.5 16 L20.5 16 L17.5 22 L16.8 28 L15.2 28 L14.5 22 Z" fill="#4fc07a"/>`,
  ...
  green: `<ellipse cx="16" cy="18" rx="13" ry="9" fill="#2f6b39"/><ellipse cx="16" cy="17" rx="9" ry="6" fill="#57b15c"/><ellipse cx="16" cy="16.5" rx="4.5" ry="3" fill="#6cc471"/><rect x="15.3" y="4" width="1.5" height="13" rx=".7" fill="#cdd6c2"/><g class="gi-wave" style="transform-origin:16px 5px"><path d="M16.6 4 L24 6.4 L16.6 9 Z" fill="#4fc07a"/></g>`,
  ...
  card: `<rect x="7" y="6" width="18" height="20" rx="2.5" fill="#e9eef0"/><rect x="7" y="6" width="18" height="5" rx="2.5" fill="#4fc07a"/><g stroke="#9aa6a8" stroke-width="1.4"><path d="M10 15 h12"/><path d="M10 19 h12"/><path d="M10 23 h8"/></g>`,
  bird: `<ellipse cx="16" cy="27" rx="8" ry="2" fill="#000" opacity=".2"/><path d="M6 16 q4 -3 8 -1 q5 -8 12 -7 q-3 3 -3 6 q3 1 3 4 q-5 5 -12 4 q-6 -1 -8 -10 Z" fill="#4fc07a"/><circle cx="22" cy="13" r="1.2" fill="#0a0f08"/><path d="M26 13 l3 -1 -3 -1 Z" fill="#ffcf5a"/>`,
  peak: `<path d="M3 27 L13 9 L17 17 L21 11 L29 27 Z" fill="#3a4a30"/><path d="M13 9 L9.5 16 L16.5 16 Z" fill="#eef3e6"/><rect x="20.5" y="4" width="1.5" height="9" rx=".7" fill="#cdd6c2"/><g class="gi-wave" style="transform-origin:21px 5px"><path d="M21.8 4 L28 6 L21.8 8.4 Z" fill="#4fc07a"/></g>`,
...
function logoMark(size = 16) {
  return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none">
    <path d="M8 21V4" stroke="#4fc07a" stroke-width="2.6" stroke-linecap="round"/>
    <path d="M8 4l9 3.5L8 11" fill="#4fc07a"/>
    <path d="M5 21c2-1.4 4.6-1.4 6.6 0" stroke="#4fc07a" stroke-width="2" stroke-linecap="round"/>
  </svg>`;
}
  ...
  return `<svg class="linechart" viewBox="0 0 ${w} ${h}">
    <defs>
      <linearGradient id="lg" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#4fc07a" stop-opacity="0.30"/>
        <stop offset="100%" stop-color="#4fc07a" stop-opacity="0"/>
      </linearGradient>
    </defs>
    <path d="${area}" fill="url(#lg)"/>
    <path d="${d}" fill="none" stroke="#4fc07a" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/>
    <circle cx="${lastX}" cy="${lastY}" r="4.5" fill="#4fc07a"/>
    <text x="${lastX + 9}" y="${lastY + 4}" fill="#4fc07a" font-size="14" font-weight="800" font-family="Inter,system-ui">${fmtToPar(points[points.length - 1])}</text>
  </svg>`;
}
...
function drillArt(key, light) {
  ...
  const flag = (x, y) => `<line x1="${x}" y1="${y}" x2="${x}" y2="78" stroke="#4fc07a" stroke-width="2"/><path d="M${x} ${y} l12 4 -12 4z" fill="#4fc07a"/>`;
  const cup = x => `<ellipse cx="${x}" cy="78" rx="8" ry="3" fill="#0a0f06" stroke="#4fc07a" stroke-width="1.5"/>`;
  ...
  if (key === 'driving') {
    ...
    return `<svg viewBox="0 0 ${W} ${H}" class="drill-art" role="img" aria-label="Tiro de salida">
      ...
      <ellipse cx="266" cy="78" rx="46" ry="6" fill="rgba(79,192,122,0.12)" stroke="rgba(79,192,122,0.4)" stroke-width="1.5" stroke-dasharray="4 4"/>
      <path d="${p}" fill="none" stroke="rgba(79,192,122,0.25)" stroke-width="2" stroke-dasharray="3 6"/>
  ...
  if (key === 'approach') {
    ...
    return `<svg viewBox="0 0 ${W} ${H}" class="drill-art" role="img" aria-label="Control de distancia">
      ${frame}${ground}
      <ellipse cx="262" cy="78" rx="40" ry="6" fill="rgba(79,192,122,0.10)"/>
      ${flag(268, 30)}
      <path d="${p}" fill="none" stroke="rgba(79,192,122,0.25)" stroke-width="2" stroke-dasharray="3 6"/>
  ...
  if (key === 'short') {
    ...
    return `<svg viewBox="0 0 ${W} ${H}" class="drill-art" role="img" aria-label="Up & down">
      ${frame}${ground}
      <ellipse cx="250" cy="78" rx="56" ry="6" fill="rgba(79,192,122,0.10)"/>
      ${cup(286)}
      <path d="${p}" fill="none" stroke="rgba(79,192,122,0.25)" stroke-width="2" stroke-dasharray="3 6"/>
  ...
  return `<svg viewBox="0 0 ${W} ${H}" class="drill-art" role="img" aria-label="Putt por el gate">
    ...
    <line x1="188" y1="58" x2="188" y2="70" stroke="#4fc07a" stroke-width="2.5" stroke-linecap="round"/>
    <line x1="208" y1="58" x2="208" y2="70" stroke="#4fc07a" stroke-width="2.5" stroke-linecap="round"/>
    <ellipse cx="286" cy="70" rx="8" ry="3" fill="#0a0f06" stroke="#4fc07a" stroke-width="1.5"/>
    <path d="${p}" fill="none" stroke="rgba(79,192,122,0.22)" stroke-width="2" stroke-dasharray="3 6"/>
...
function drillScene(name, areaKey) {
  ...
  const fl = (x, y) => `<line x1="${x}" y1="${y}" x2="${x}" y2="${G}" stroke="#7fb52b" stroke-width="2"/><path d="M${x} ${y} l11 3.5 -11 3.5z" fill="#4fc07a"/>`;
  ...
  if (n.includes('gate drill')) return svg(`${gr()}<line x1="74" y1="62" x2="74" y2="${G}" stroke="#4fc07a" stroke-width="3" stroke-linecap="round"/><line x1="100" y1="62" x2="100" y2="${G}" stroke="#4fc07a" stroke-width="3" stroke-linecap="round"/><ellipse cx="272" cy="${G}" rx="28" ry="5" fill="rgba(79,192,122,0.12)"/>${bl('M44 74 Q 170 -10 286 74', '3s')}${cap('Gate de alineación')}`);
  if (n.includes('14 calles')) return svg(`${gr()}<rect x="150" y="71" width="120" height="7" rx="3.5" fill="#2f6b39"/><g stroke="#4fc07a" stroke-width="2"><line x1="180" y1="67" x2="180" y2="74"/><line x1="210" y1="67" x2="210" y2="74"/><line x1="240" y1="67" x2="240" y2="74"/></g>${bl('M34 74 Q 150 -8 210 72', '2.6s')}${cap('14 calles a presión')}`);
  if (n.includes('madera 3')) return svg(`${gr()}<path d="M44 74 Q 150 -12 256 58" fill="none" stroke="rgba(255,120,107,0.3)" stroke-dasharray="3 5"/><path d="M44 74 Q 150 2 256 72" fill="none" stroke="rgba(79,192,122,0.3)" stroke-dasharray="3 5"/>${bl('M44 74 Q 150 -12 256 58', '3s')}<circle r="4" fill="#4fc07a"><animateMotion dur="3s" begin="1.3s" repeatCount="indefinite" calcMode="linear" path="M44 74 Q 150 2 256 72"/></circle>${cap('Driver vs Madera 3')}`);
  if (n.includes('escalera')) return svg(`${gr()}${[120, 200, 268].map((x, i) => `<ellipse cx="${x}" cy="${G}" rx="13" ry="4" fill="rgba(79,192,122,0.12)"/><circle r="4" fill="#fff"><animateMotion dur="3s" begin="${i * 0.6}s" repeatCount="indefinite" calcMode="linear" path="M34 74 Q ${(34 + x) / 2} -6 ${x} 74"/><animate attributeName="opacity" values="0;1;1;0;0" keyTimes="0;.1;.45;.55;1" dur="3s" begin="${i * 0.6}s" repeatCount="indefinite"/></circle>`).join('')}${cap('Escalera de distancias')}`);
  if (n.includes('reloj con wedges')) return svg(`<circle cx="64" cy="50" r="26" fill="none" stroke="rgba(79,192,122,0.28)" stroke-width="2"/><circle cx="64" cy="50" r="2.5" fill="#4fc07a"/><line x1="64" y1="50" x2="64" y2="28" stroke="#4fc07a" stroke-width="3" stroke-linecap="round"><animateTransform attributeName="transform" type="rotate" values="-35 64 50;-95 64 50;-35 64 50" dur="2.6s" repeatCount="indefinite"/></line>${fl(268, 46)}${bl('M118 60 Q 200 8 260 72', '2.6s')}${cap('Reloj de wedges')}`);
  if (n.includes('pin high')) return svg(`${gr()}${fl(244, 40)}<line x1="120" y1="62" x2="266" y2="62" stroke="rgba(79,192,122,0.25)" stroke-width="1.5" stroke-dasharray="3 5"/>${bl('M34 74 Q 150 -2 232 62', '2.8s')}${cap('Pin high siempre')}`);
  ...
  if (n.includes('lag putting')) return svg(`${gr()}${cp(252)}<circle cx="252" cy="${G}" r="22" fill="none" stroke="rgba(79,192,122,0.32)" stroke-width="1.2" stroke-dasharray="3 3"/><path d="M40 ${G} L236 ${G}" stroke="rgba(79,192,122,0.18)" stroke-dasharray="3 6"/><circle r="4.5" fill="#fff"><animateMotion dur="3s" repeatCount="indefinite" calcMode="linear" keyPoints="0;1;1" keyTimes="0;.66;1" path="M40 ${G} L236 ${G}"/></circle>${cap('Lag a círculo de 1 m')}`);
  if (n.includes('gate de putter')) return svg(`${gr(70)}<line x1="190" y1="58" x2="190" y2="70" stroke="#4fc07a" stroke-width="2.5" stroke-linecap="round"/><line x1="210" y1="58" x2="210" y2="70" stroke="#4fc07a" stroke-width="2.5" stroke-linecap="round"/><ellipse cx="272" cy="70" rx="8" ry="3" fill="#0a0f06" stroke="#4fc07a" stroke-width="1.5"/><path d="M40 70 L272 70" stroke="rgba(79,192,122,0.2)" stroke-dasharray="3 6"/><circle fill="#fff"><animateMotion dur="2.4s" repeatCount="indefinite" calcMode="linear" keyPoints="0;1;1" keyTimes="0;.72;1" path="M40 70 L272 70"/><animate attributeName="r" values="4.5;4.5;0;0" keyTimes="0;.68;.74;1" dur="2.4s" repeatCount="indefinite"/></circle>${cap('Gate de putter')}`);
  if (n.includes('reloj de 1.5')) {
    const cx = 160, cy = 46, R = 30;
    const dots = [0, 1, 2, 3, 4, 5, 6, 7].map(i => { const a = i / 8 * Math.PI * 2; return `<circle cx="${(cx + Math.cos(a) * R).toFixed(0)}" cy="${(cy + Math.sin(a) * R * 0.7).toFixed(0)}" r="2.2" fill="rgba(79,192,122,0.4)"/>`; }).join('');
    return svg(`<ellipse cx="${cx}" cy="${cy}" rx="8" ry="4" fill="#0a0f06" stroke="#4fc07a" stroke-width="1.5"/>${dots}<circle fill="#fff"><animateMotion dur="2.2s" repeatCount="indefinite" calcMode="linear" keyPoints="0;1;1" keyTimes="0;.6;1" path="M${cx + R} ${cy} L${cx} ${cy}"/><animate attributeName="r" values="4.5;4.5;0;0" keyTimes="0;.56;.62;1" dur="2.2s" repeatCount="indefinite"/></circle>${cap('Reloj de 1.5 m')}`);
  }
...
function regScene(kind) {
  ...
  if (kind === 'gir') return wrap(`<ellipse cx="42" cy="33" rx="14" ry="6.5" fill="#54ad58"/><ellipse cx="38" cy="31" rx="6" ry="2.6" fill="#79c970" opacity=".6"/><line x1="42" y1="32" x2="42" y2="13" stroke="#2c4a1c" stroke-width="1.7"/><path d="M42,13 l9,2.6 -9,2.6z" fill="#4fc07a"/><path d="M5,34 Q26,-3 42,29" fill="none" stroke="rgba(20,40,10,.18)" stroke-width="1.4" stroke-dasharray="2 3"/>${ball('M5,34 Q26,-3 42,29', '2.4s')}`);
  if (kind === 'ud') return wrap(`<ellipse cx="46" cy="34" rx="10" ry="4.5" fill="#54ad58"/><ellipse cx="46" cy="34" rx="3" ry="1.5" fill="#0a2e16"/><line x1="46" y1="34" x2="46" y2="16" stroke="#2c4a1c" stroke-width="1.6"/><path d="M46,16 l8,2.3 -8,2.3z" fill="#4fc07a"/><ellipse cx="8" cy="36" rx="6" ry="2.4" fill="#5a8a3c"/>${ball('M8,34 Q26,8 42,30 L46,34', '2.8s')}`);
  ...
  return wrap(`<path d="M8,32 L46,32" stroke="rgba(20,40,10,.18)" stroke-width="1.4" stroke-dasharray="2 3"/><ellipse cx="50" cy="32" rx="6.5" ry="2.6" fill="#0a2e16"/><line x1="50" y1="32" x2="50" y2="14" stroke="#2c4a1c" stroke-width="1.6"/><path d="M50,14 l9,2.6 -9,2.6z" fill="#4fc07a"/><circle r="3.4" fill="#fff" stroke="#16301a" stroke-width="0.7"><animateMotion dur="2.4s" repeatCount="indefinite" calcMode="linear" keyPoints="0;1;1" keyTimes="0;.72;1" path="M8,32 L48,32"/><animate attributeName="r" values="3.4;3.4;0;0" keyTimes="0;.7;.76;1" dur="2.4s" repeatCount="indefinite"/></circle>`);
}
